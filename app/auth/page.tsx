...
export default function AuthPage() {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'Create Account' && password !== confirmPassword) {
      setAuthError('Passwords do not match.');
      return;
    }
    setAuthLoading(true);
    // Simulate auth check — redirect after brief delay
    setTimeout(() => {
        ...
        <section className="bg-black text-white p-8 md:p-16 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 bg-zinc-950 border border-zinc-900 rounded-2xl p-8 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              {authError && (
                <div className="text-red-400 text-[10px] font-black uppercase tracking-widest text-center py-2">{authError}</div>
              )}
              {/* Core CTA */}
              <button type="submit" disabled={authLoading} className="w-full py-3.5 bg-[#FFBE00] hover:bg-amber-500 text-black text-xs font-extrabold tracking-widest uppercase rounded-lg transition-all shadow-md mt-2">
                {authLoading ? 'AUTHENTICATING...' : activeTab === 'Create Account' ? 'CREATE ACCOUNT & START' : 'SIGN IN TO WORKSPACE'}
              </button>
